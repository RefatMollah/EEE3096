...
int main(void){
  ...
  while (1)
  {
	...
	getTime();
	epochNow = epochFromTime(time);
	sprintf(buffer, "Epoch Time: %u \r\n",epochNow);
	HAL_UART_Transmit(&huart2, buffer, sizeof (buffer), 1000);
	sprintf(buffer2,"Date: %d/%d/%d Time: %d:%d:%d \r\n" ,time.year,time.month,time.dayofmonth,time.hour,time.minutes,time.seconds);
...
void getTime (void)
{
	...
	//YOUR CODE HERE
	HAL_I2C_Mem_Read(&hi2c1, 0xD0, 0x00, 1, get_time, 7, 100);
